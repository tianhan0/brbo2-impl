...
object PathRefinementTestCases {
  ...
  private val use1 = CFGNode(Left(Use(Some(1), update, assignment)), brboProgram.mainFunction, CFGNode.DONT_CARE_ID)
  private val use2 = CFGNode(Left(Use(Some(2), update, assignment)), brboProgram.mainFunction, CFGNode.DONT_CARE_ID)
