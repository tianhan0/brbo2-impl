...
object PathRefinementTestCases {
  ...
  private val r1Initialization = GhostVariableUtils.declareVariables(1).map(c => CFGNode(Left(c), brboProgram.mainFunction, CFGNode.DONT_CARE_ID))
  private val r2Initialization = GhostVariableUtils.declareVariables(2).map(c => CFGNode(Left(c), brboProgram.mainFunction, CFGNode.DONT_CARE_ID))
