...
object BoundAssertion {
  def parse(tag: BrboExpr, expr: BrboExpr): BoundAssertion = {
    val allResourceIdentifiers = BrboExprUtils.collectIdentifiers(expr).filter(i => GhostVariableUtils.isGhostVariable(i.identifier, GhostVariableTyp.Resource))
    if (allResourceIdentifiers.isEmpty || allResourceIdentifiers.size > 1) {
      ...
      tag match {
        case StringLiteral(value, _) =>
          BoundAssertion(allResourceIdentifiers.head.identifier, expr, value)
