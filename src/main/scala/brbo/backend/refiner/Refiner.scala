...
class Refiner(boundAssertion: BrboExpr, brboProgram: BrboProgram, commandLineArguments: CommandLineArguments) {
  private val logger = MyLogger.createLogger(classOf[Refiner], commandLineArguments.getDebugMode)
  private val pathTransformation = new PathTransformation(commandLineArguments, brboProgram)
    ...
    counterexamplePath match {
      case Some(counterexamplePath2) =>
        ...
        while (newProgram.isEmpty) {
          ...
          newProgram = programTransformation.programTransformation(brboProgram2, paths)
        }
        newProgram.get
